...
The same commands work on both Windows and Linux (the root `dev.py` auto-detects the JBR / SDK / ADB per platform):

```bash
# Build a debug APK (--clean optional)
python dev.py build debug

# Build a release APK
python dev.py build release

# Build and install to a device (wireless adb supported)
python dev.py install debug
```

Or use the Gradle Wrapper directly (recommended for CI):

```bash
./gradlew :app:assembleDebug --console=plain    # gradlew.bat on Windows
```
...
See the [Cross-Platform Development Guide](docs/development/cross-platform-dev.md) for environment setup, the one-command environment check, and Go AAR builds.
