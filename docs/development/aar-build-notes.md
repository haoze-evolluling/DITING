...
> **跨平台快捷方式**：Windows / Linux 均可直接运行 `python dev.py aar`，脚本会自动探测 SDK 与 JBR、设置 `ANDROID_HOME` / `JAVA_HOME` / `GOFLAGS=-buildvcs=false` 并按本文参数执行 `gomobile bind`，无需手动配置环境变量。下文为手动构建过程的完整记录（详见 [跨平台开发指南](cross-platform-dev.md)）。
