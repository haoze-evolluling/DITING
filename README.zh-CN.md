...
Windows 与 Linux 使用同一套命令（根目录 `dev.py` 会自动探测 JBR / SDK / ADB 并适配平台差异）：

```bash
# 编译 Debug APK（--clean 可选）
...
跨平台环境配置、一键环境体检与 Go AAR 构建详见 [跨平台开发指南](docs/development/cross-platform-dev.md)。
