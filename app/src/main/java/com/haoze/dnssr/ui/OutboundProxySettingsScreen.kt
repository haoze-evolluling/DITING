...
fun OutboundProxyAppsScreen(onBack: () -> Unit, onSave: (String) -> Unit) {
    val context = LocalContext.current
    var selectedPackage by remember { mutableStateOf(AppSettings.getOutboundProxyConfig(context).proxyAppPackage) }
    ...
    SettingsScaffold(title = localizedText("选择代理应用"), onBack = onBack, actions = {
        AppListOverflowMenu(
            filter = filter,
            sort = sort,
            onSelectAll = {},
        ...
        Column(Modifier.fillMaxSize().padding(padding), verticalArrangement = Arrangement.spacedBy(12.dp)) {
            SettingsInfoText(localizedText("选择提供本地代理端口的应用。代理应用本身会绕过 DNSSR。"), Modifier.padding(top = 8.dp))
            OutlinedTextField(
                value = query,
            ...
            SettingsActionButton(
                onClick = { onSave(selectedPackage) },
