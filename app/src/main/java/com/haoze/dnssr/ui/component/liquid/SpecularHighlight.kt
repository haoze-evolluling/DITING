...
/**
 * Builds a highlight whose primary light source follows the device gravity
 * direction (rotated by [extraDegrees]).
 *
 * Pass a shared [tiltState] when multiple highlights render together so they
 * consume a single sensor registration instead of one each.
 */
@Composable
