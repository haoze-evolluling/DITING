...
private fun englishGuide(guide: SettingsGuide): SettingsGuide = when (guide.id) {
    ...
    "config_transfer" -> guide.copy(
        ...
        message = "Import and export lets you move DNSSR custom configuration between devices, including providers, app settings, and rule subscriptions. Exported files may contain service addresses or other personal settings, so store them carefully.\n\nImport can replace or merge corresponding local data depending on the selected option. Export a backup first and confirm that the file source is trusted and compatible.\n\nReview important settings after the operation and restart the service when necessary."
    )
    ...
    "blocked_apps" -> guide.copy(
        title = "Blocked apps",
        message = "Blocked apps cannot connect through the local VPN created by DNSSR. Use this to temporarily restrict apps that should not access the network.\n\nThis feature requires Android 10 or later. On older versions the switch and app selector remain disabled.\n\nWhen enabled, network requests, background sync, notifications, and account verification for selected apps may fail. Remove an app from the list to restore normal forwarding."
    )
    "app_allowlist" -> guide.copy(
        title = "App allowlist access",
        message = "App allowlist access restricts selected apps to IP addresses resolved from their allowlisted domains. A root domain also matches subdomains, and entries expire automatically when their DNS TTL expires.\n\nDirect IP connections, LAN access, encrypted DNS, and unauthorized connections are blocked. Apps sharing the same UID are restricted together, and a shared CDN IP may serve other sites during its valid TTL.\n\nThis feature requires Android 10 or later and enables the Go full-traffic tunnel."
    )
...
object SettingsGuides {
    ...
    val CONFIG_TRANSFER = SettingsGuide("config_transfer", "导入与导出", "导入与导出用于在设备之间迁移谛听的自定义配置，包括服务商、应用设置以及规则订阅等内容。导出前请留意文件中可能包含的服务地址或其他个人配置，并妥善保存。\n\n导入时系统会按所选方式覆盖或合并本机对应数据，因此建议先导出现有配置作为备份，并确认文件来源可信、版本兼容。\n\n操作完成后请检查关键设置，必要时重新启动服务使配置生效。")
    ...
    val BLOCKED_APPS = SettingsGuide("blocked_apps", "禁止联网应用", "禁止联网应用用于阻止选中的应用通过谛听所建立的本地 VPN 连接网络，适合临时限制不希望联网的应用。\n\n此功能需要 Android 10 或更高版本。系统版本不满足要求时无法使用，页面内的开关和应用选择入口会保持灰色。\n\n启用后，所选应用的网络请求、后台同步、消息通知和账号验证都可能失败；取消选择后即可恢复正常转发。")
    val APP_ALLOWLIST = SettingsGuide("app_allowlist", "应用白名单访问", "应用白名单访问会限制选中应用只连接其白名单域名解析出的 IP。白名单根域同时匹配子域名，记录会在 DNS TTL 到期后自动失效。\n\n直连 IP、局域网、加密 DNS 和未授权连接会被阻止。共享同一 UID 的应用会一并受限；共享 CDN IP 在有效 TTL 内可能承载其他站点。\n\n此功能需要 Android 10 或更高版本，并会启用 Go 全流量隧道。")
