            ...
            SettingsSurfaceGroup(
                content = buildList {
                    ...
                    if (update != null) {
                        add {
                            SettingsTextItem(
                                title = localizedText(if (downloadStatus == AppUpdateDownloadStatus.Downloaded) "安装 ${update.version}" else "下载 ${update.version}"),
...
private fun updateStatusText(state: AppUpdateUiState, downloadStatus: AppUpdateDownloadStatus): String = when {
    ...
    state.availableUpdate != null -> localizedText("发现 ${state.availableUpdate.version} 新版本，${downloadActionText(downloadStatus)}")
