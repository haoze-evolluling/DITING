...
fun HttpInspectionAppsScreen(onBack: () -> Unit) {
    ...
    SettingsScaffold(title = localizedText("选择检查应用"), onBack = onBack, actions = {
        ...
        Column(Modifier.fillMaxSize().padding(innerPadding), verticalArrangement = Arrangement.spacedBy(12.dp)) {
            SettingsInfoText(localizedText("Go 隧道接管流量后，HTTPS 流量检查仅检查所选应用的 HTTP(S) 请求，其他应用直接转发。选择应用会取消其“排除应用”状态。"), Modifier.padding(top = 8.dp))
            ...
            SettingsActionButton(onClick = {
                ...
                onBack()
            }, modifier = Modifier.fillMaxWidth().padding(horizontal = 16.dp, vertical = 8.dp)) { Text(localizedText("保存")) }
        }
    }
}
