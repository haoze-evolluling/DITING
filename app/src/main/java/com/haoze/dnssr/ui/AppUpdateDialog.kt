...
import com.haoze.dnssr.update.AppUpdateInfo
import com.haoze.dnssr.update.formatAppUpdateBytes
...
private fun ReleaseNotes(releaseNotes: String) {
    ...
    LazyColumn(modifier = Modifier.heightIn(max = 320.dp)) {
        if (lines.isEmpty()) {
            item { Text(localizedText("本次更新暂未提供详细说明。"), style = MaterialTheme.typography.bodyMedium) }
        } else {
            items(lines) { line ->
                Text(
