...
class SettingsRouteActivity : AppLocalizedActivity() {
    ...
    private fun SettingsRouteContent(
        ...
        when (route) {
            ...
            Routes.ADDRESS_RULE_MANAGEMENT -> SettingsGuideHost(SettingsGuides.DOMAIN_RULES) { RuleManagementScreen(onBack, RuleScope.HTTPS, "地址规则", onNavigateToRuleList = onBack, onNavigateToAllowRuleList = onBack, onNavigateToRewriteRuleList = { onNavigate(Routes.HTTPS_REWRITE_RULE_LIST) }, onNavigateToUrlRuleList = { onNavigate(Routes.ADDRESS_RULE_LIST) }, onNavigateToUrlAllowRuleList = { onNavigate(Routes.ADDRESS_ALLOW_RULE_LIST) }, onNavigateToSubscription = onBack, onNavigateToMirrorTemplates = onBack, onNavigateToAutoUpdateInterval = onBack, onNavigateToBlockResponseSettings = onBack, onRuntimeDnsSettingsChanged = onRuntimeDnsSettingsChanged, addressOnly = true) }
             ...
             Routes.HTTPS_REWRITE_RULE_LIST -> RuleListScreen(onBack, ruleKind = ManagedRuleKind.REWRITE, ruleScope = RuleScope.HTTPS, onRuntimeDnsSettingsChanged = onRuntimeDnsSettingsChanged)
            ...
            Routes.APP_ALLOWLIST -> SettingsGuideHost(SettingsGuides.APP_ALLOWLIST) { AppAllowlistSettingsScreen(onBack, { onNavigate(Routes.APP_ALLOWLIST_SELECTION) }) }
            Routes.APP_ALLOWLIST_SELECTION -> AppAllowlistAppsScreen(onBack)
            Routes.BOOTSTRAP_SETTINGS -> SettingsGuideHost(SettingsGuides.BOOTSTRAP) { BootstrapSettingsScreen(onBack, ScreenDestinations.bootstrapSettings.title) }
            ...
            Routes.HTTP_INSPECTION_SETTINGS -> HttpInspectionSettingsScreen(onBack, { onNavigate(Routes.HTTP_REQUEST_LOGS) }, { onNavigate(Routes.HTTP_INSPECTION_APPS) }, { onNavigate(Routes.CA_CERTIFICATE_SETTINGS) })
            ...
            Routes.NOTIFICATION_TEXT_SETTINGS -> NotificationTextSettingsScreen(onBack, "通知栏文案")
            Routes.GO_TUNNEL_DISABLE_BUTTON_SETTINGS -> GoTunnelDisableButtonSettingsScreen(onBack, "Go 隧道关闭按钮")
            ...
            Routes.LEGACY_ICON_SETTINGS -> LegacyIconSettingsScreen(onBack, "旧版图标")
            Routes.LEGACY_LOG_PAGE_SETTINGS -> LegacyLogPageSettingsScreen(onBack, "旧版日志页面")
