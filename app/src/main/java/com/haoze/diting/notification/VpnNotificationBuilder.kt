...
object VpnNotificationBuilder {
    ...
    fun build(
        ...
        val proxyEnabled = OutboundProxySettingsStore.isOutboundProxyEnabled(context)
        val proxyStatus = OutboundProxySettingsStore.getOutboundProxyStatus(context)

        // 1. Build the primary status text
        val primaryText = when {
            proxyEnabled && proxyStatus.first == "error" -> {
                val errorMsg = OutboundProxySettingsStore.formatProxyErrorMessage(context, proxyStatus.second)
                "${localizedText(context, "出站代理不可用")} · $errorMsg"
            }
            proxyEnabled && proxyStatus.first == "connecting" ->
