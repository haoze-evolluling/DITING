...
interface BlockRuleDao {
    ...
    @Query(
        "SELECT id FROM block_rule WHERE pattern = :pattern AND important = :important " +
            ...
            "AND ((:dnsType IS NULL AND dnsType IS NULL) OR dnsType = :dnsType) AND isRegex = :isRegex"
    )
    ...
    @Query(
        "SELECT id, pattern, important, appScope, appInverted, dnsType, isRegex FROM block_rule " +
    ...
    suspend fun insertForSource(entity: BlockRuleEntity, source: String, sourceEnabled: Boolean): Boolean {
        ...
        val ruleId = if (insertedId == -1L) {
            idByPattern(entity.pattern, entity.important, entity.appScope, entity.appInverted, entity.dnsType, entity.isRegex)
        } else insertedId
    ...
    suspend fun insertAllForSource(
            ...
            for (i in conflictedIndices) {
                ...
                val matched = candidates?.firstOrNull {
                    it.important == entity.important &&
                        ...
                        it.dnsType == entity.dnsType &&
                        it.isRegex == entity.isRegex
                }
        ...
        val sourceEntities = ArrayList<BlockRuleSourceEntity>(entities.size)
        for (i in entities.indices) {
            sourceEntities.add(BlockRuleSourceEntity(ruleIds[i], source, sourceEnabled))
        }
        return insertSources(sourceEntities).count { it != -1L }
    }
    ...
    @Query(
        "SELECT r.id, r.pattern, r.rawLine, r.addedAt, " +
            ...
            "r.groupName, r.important, r.appScope, r.appInverted, r.isWildcard, r.denyallow, r.isRegex, r.dnsType " +
    ...
    @Query(
        "SELECT r.id, r.pattern, r.rawLine, r.addedAt, " +
            ...
            "r.groupName, r.important, r.appScope, r.appInverted, r.isWildcard, r.denyallow, r.isRegex, r.dnsType " +
            "FROM block_rule r " +
            "WHERE pattern = :pattern AND important = :important " +
            "AND ((:appScope IS NULL AND appScope IS NULL) OR appScope = :appScope) AND appInverted = :appInverted"
    )
    suspend fun ruleByPattern(
        pattern: String,
        important: Boolean,
        appScope: String? = null,
        appInverted: Boolean = false
    ...
    suspend fun rulesByPattern(pattern: String): List<BlockRuleEntity>

    @Query(
        "SELECT r.id AS id, r.pattern, MIN(s.source) AS source, r.important AS important, " +
            "r.appScope AS appScope, r.appInverted AS appInverted, r.isWildcard AS isWildcard, " +
            "r.denyallow AS denyallow, r.isRegex AS isRegex, r.dnsType AS dnsType FROM block_rule r " +
            "JOIN block_rule_source s ON s.ruleId = r.id " +
            "WHERE r.id > :lastId AND r.enabled = 1 AND s.enabled = 1 AND s.source LIKE 'sub_%' " +
            "AND r.important = :important GROUP BY r.id, r.pattern, r.important, r.appScope, r.appInverted, r.isWildcard, r.denyallow, r.isRegex, r.dnsType " +
    ...
    @Query(
        "SELECT r.pattern, MIN(s.source) AS source, r.important AS important, " +
            ...
            "GROUP BY r.id, r.pattern, r.important, r.appScope, r.appInverted, r.isWildcard, r.denyallow, r.isRegex, r.dnsType"
    )
    ...
    @Query(
        "SELECT r.pattern, MIN(s.source) AS source, r.important AS important, " +
            ...
            "AND (r.isWildcard = 1 OR r.pattern LIKE '%*%' OR r.appScope IS NOT NULL OR r.appInverted = 1 OR r.isRegex = 1 OR r.denyallow IS NOT NULL OR r.dnsType IS NOT NULL) " +
            "GROUP BY r.id, r.pattern, r.important, r.appScope, r.appInverted, r.isWildcard, r.denyallow, r.isRegex, r.dnsType"
    )
