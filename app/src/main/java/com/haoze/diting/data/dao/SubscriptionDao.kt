...
interface SubscriptionDao {
    ...
    @Query(
        "SELECT sub.* FROM subscription sub LEFT JOIN subscription_group grp ON grp.id = sub.groupId " +
            "WHERE sub.sourceType = 'remote' AND sub.enabled = 1 AND (sub.groupId IS NULL OR grp.autoUpdateEnabled = 1) ORDER BY sub.addedAt DESC"
    )
