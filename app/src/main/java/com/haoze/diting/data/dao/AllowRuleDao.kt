...
interface AllowRuleDao {
    ...
    @Query(
        "SELECT id, pattern, important, appScope, appInverted, dnsType, isRegex FROM allow_rule " +
    ...
    suspend fun insertAllForSource(
        ...
        val sourceEntities = ArrayList<AllowRuleSourceEntity>(entities.size)
        for (i in entities.indices) {
            sourceEntities.add(AllowRuleSourceEntity(ruleIds[i], source, sourceEnabled))
        }
    ...
    @Query(
        "SELECT r.id, r.pattern, r.rawLine, r.addedAt, " +
            ...
            "r.groupName, r.appScope, r.appInverted, r.isWildcard, r.important, r.denyallow, r.isRegex, r.dnsType " +
    ...
    @Query(
        "SELECT r.id AS id, r.pattern, MIN(s.source) AS source, r.important AS important, " +
            ...
            "r.denyallow AS denyallow, r.isRegex AS isRegex, r.dnsType AS dnsType FROM allow_rule r " +
