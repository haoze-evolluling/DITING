...
/**
 * Localization entries for the about screen, open-source repositories, version updates, the disclaimer, sponsorship, and contributor lists.
 */
internal fun translateAboutAndUpdateExact(text: String): String? = when (text) {
    "使用前说明" -> "Before you begin"
    "感谢每一位为谛听提出建议、帮助测试的共建者！名单按用户名称的字母顺序排列，中文名称按拼音排序。" -> "Thanks to every DITING co-builder who contributed suggestions and testing help. The list is sorted alphabetically by username, with Chinese names sorted by pinyin."
    ...
    "谛听 / RESOLUTION UNDER YOUR CONTROL" -> "DITING / RESOLUTION UNDER YOUR CONTROL"
    ...
    "感谢每一位支持谛听项目的朋友！名单默认按赞助时间由早到晚排列，可通过右上角按钮切换为由晚到早；与赞助金额无关，每一份支持都同样珍贵。" -> "Thanks to everyone who supports the DITING project! The list is ordered by sponsorship time (earliest first) by default; tap the button in the top-right corner to reverse it. Regardless of amount, every bit of support is equally precious."
    ("软件说明\n" +
        ...
        ("Software Overview\n" +
            "DITING is a DNS management tool based on Android local VPN. This software is intended to block and filter harmful domains and purify the network environment; it is not intended for filtering commercial advertisements.\n\n" +
            "Important Notes\n" +
            "The software relies on local VPN and upstream DNS services to run. Resolution results are affected by network conditions and user configurations; rules and logs are stored locally on the device. Enabling extended features may alter network behavior, so please configure with caution after understanding their effects.\n\n" +
            "Disclaimer\n" +
            "This software is provided \"as is\". Users must comply with relevant laws and regulations, and independently verify the legitimacy and safety of rules and upstream sources. Any illegal use is strictly prohibited. Any consequences arising from misuse of the software or using it for other purposes shall be borne solely by the user.")
    else -> null
}

internal fun translateAboutAndUpdatePattern(text: String): String? = when {
    text.startsWith("感谢您对谛听项目的赞助支持") -> "Thanks for supporting the DITING project"
