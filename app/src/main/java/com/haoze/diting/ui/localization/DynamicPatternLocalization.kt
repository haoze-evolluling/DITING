...
/**
 * Cross-module dynamic template substitution and formatting helpers.
 */
internal fun translateDynamicPattern(text: String): String? = when {
    ...
    text.startsWith("感谢为谛听提出建议与帮助测试") -> "Thanks for suggestions and testing help for DITING"
    ...
    text.startsWith("谛听需要建立本地 VPN") -> "DITING needs a local VPN to process and filter DNS requests. This permission lets the app handle DNS traffic on the device; it does not send all network traffic to a remote VPN server."
    ...
    // List updates, config-import logs, and summaries
    ...
    // Config import details: settings toggles and list-type logs
    ...
    // Dashboard direct-connection labels (combinations like "UDP direct · pkg")
