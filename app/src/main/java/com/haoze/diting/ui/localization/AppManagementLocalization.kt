...
/**
 * Localization entries for the app whitelist, network-blocked apps, excluded apps, app permissions, and app-list sorting/filtering.
 */
internal fun translateAppManagementExact(text: String): String? = when (text) {
    ...
    "为了让你选择需要排除或进行 HTTP(S) 检查的应用，谛听需要读取设备上的应用列表。不会读取应用数据，也不会上传应用列表。" -> "DITING needs access to the installed app list so you can choose apps to exclude or inspect for HTTP(S) traffic. It does not read app data or upload the app list."
    ...
    // Per-app rules and full outbound blocking
