...
/**
 * Localization entries for HTTPS traffic inspection, CA root-certificate status, certificate fingerprints, install/uninstall guides, and QUIC/DoT blocking.
 */
internal fun translateHttpInspectionExact(text: String): String? = when (text) {
    ...
    "HTTP CONNECT 不支持 UDP；UDP 会被严格阻断，QUIC 可回退到 TCP。代理应用本身将绕过 DITING。" -> "HTTP CONNECT does not support UDP; UDP is strictly blocked, and QUIC can fall back to TCP. The proxy app itself bypasses DITING."
...
internal fun translateHttpInspectionPattern(text: String): String? = when {
    text.startsWith("“已放行/已拦截”表示") -> "“Allowed/Blocked” means DITING has read the HTTP request's authority and matched it against the existing domain rules.\n\n" +
    ...
    text.startsWith("请在系统设置中选择“从存储设备安装 CA 证书”") -> {
                ...
                "In system settings, choose “Install CA certificate from storage”, then select $fileName. Return here and DITING will verify the installation status. Connections using certificate pinning or custom validation will be bypassed automatically."
            }
