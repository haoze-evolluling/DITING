...
private fun englishGuide(guide: SettingsGuide): SettingsGuide = when (guide.id) {
    ...
    "log_mode" -> guide.copy(
        title = "Log mode",
        message = "Log mode controls which DNS requests DITING records and how much troubleshooting detail is retained. More complete logs can identify the requesting app, matched rule, and responding provider, but use more storage and leave more local access records.\n\nLong-press the Logs card in the home feature menu to open these settings directly.\n\nUse a less detailed range when you only need service status, and increase detail temporarily when investigating a rule or resolution issue. Logs stay on this device and can be deleted from Data cleanup."
    )
    "config_transfer" -> guide.copy(
        title = "Import and export",
        message = "Import and export lets you move DITING custom configuration and custom rules between devices, including providers, custom rules, app settings, and rule subscriptions. Exported files may contain service addresses or other personal settings, so store them carefully.\n\nImport will merge corresponding local data and skip existing items. Export a backup first and confirm that the file source is trusted and compatible.\n\nReview important settings after the operation and restart the service when necessary."
    )
    "domain_rules" -> guide.copy(
        title = "Domain rules guide",
        warning = "This software is intended to block and filter harmful domains and purify the network environment; it is not intended for filtering commercial advertisements.",
        message = "Usage Instructions\n" +
            "Domain rules control blocking, allowing, and rewriting domains, directly affecting network access for apps and websites. Please configure rules reasonably to avoid service access issues caused by rule conflicts or overly broad patterns.\n\n" +
            "Important Notes\n" +
            "Rule data is stored locally on the device. Resolution performance is influenced by network conditions and rule configurations. After adding or updating rules, it is recommended to observe actual hits via request logs.",
        acknowledgement = null,
        nextStep = guide.nextStep?.copy(
            title = "Disclaimer and Compliance",
            warning = "Any unlawful or unauthorized use of this software is strictly prohibited.",
            message = "Disclaimer\n" +
                "This software is provided \"as is\". Users must strictly comply with relevant laws and regulations and independently verify the legitimacy and safety of rule sources. Any unlawful use is strictly prohibited.\n\n" +
                "Usage and Liability Statement\n" +
                "This software is intended to block and filter harmful domains and purify the network environment; it is not intended for filtering commercial advertisements. Any consequences arising from misuse of the software or using it for other purposes shall be borne solely by the user.",
            acknowledgement = "I have read and understood all terms. Any consequences arising from misuse of the software or using it for other purposes shall be borne solely by the user."
        )
    )
    ...
    "foreground_background" -> guide.copy(
        ...
        message = "These settings control how DITING presents and maintains its operation after leaving the foreground, including persistent notifications, recent-task visibility, and related system interactions.\n\nSome options affect how the system manages background services. Hiding the interface does not mean that the service has stopped, especially on devices with aggressive battery policies.\n\nDecide whether you want to hide the app, reduce notifications, or keep DNS service stable. Test the result by locking the screen or switching apps."
    )
    "excluded_apps" -> guide.copy(
        title = "Excluded apps",
        message = "Excluded apps continue using the system DNS instead of DITING's DNS resolver.\n\nTheir domain requests usually do not match DITING block, allow, or rewrite rules, and their logs and statistics may be incomplete. This is useful for apps that are incompatible with the local VPN, a particular network, or a specific resolver.\n\nFiltering not working after adding an app is expected. Remove it from the list to restore DITING processing."
    )
    "blocked_apps" -> guide.copy(
        title = "Blocked apps",
        message = "Blocked apps cannot connect through the local VPN created by DITING. Use this to temporarily restrict apps that should not access the network.\n\nWhen enabled, network requests, background sync, notifications, and account verification for selected apps may fail. Remove an app from the list to restore normal forwarding."
    )
...
object SettingsGuides {
    ...
    val DOMAIN_RULES = SettingsGuide(
        id = "domain_rules",
        title = "域名规则使用说明",
        warning = "本软件旨在屏蔽、过滤有害域名，净化网络环境，并不用于过滤商业广告。",
        message = "使用说明\n" +
            "域名规则用于控制域名的拦截、放行与重写，直接影响相关应用与网站的网络访问。请合理配置规则，避免因规则冲突或范围过宽导致服务访问异常。\n\n" +
            "注意事项\n" +
            "规则数据均保存在设备本地。解析表现受网络环境与规则配置影响，新增或更新规则后，建议通过请求日志观察实际命中情况。",
        acknowledgement = null,
        nextStep = SettingsGuide(
            id = "domain_rules_step2",
            title = "免责条款与合规声明",
            warning = "严禁将本软件用于任何违法违规用途，使用者须自负其责。",
            message = "免责条款\n" +
                "本软件按现状提供。使用者须严格遵守国家相关法律法规，自行确认规则来源的合法性与安全性，严禁将本软件用于任何违法违规活动。\n\n" +
                "用途与责任声明\n" +
                "本软件旨在屏蔽、过滤有害域名，净化网络环境，并不用于过滤商业广告。对于滥用软件或将其用于其他用途所产生的后果，由使用者自行承担。",
            acknowledgement = "我已阅读并理解全部条款，对于滥用软件或将其用于其他用途所产生的后果，由使用者自行承担。"
        )
    )
...
fun SettingsGuideHost(
    ...
    var currentStep by remember(guide.id) { mutableStateOf(1) }
    var acknowledged by remember(guide.id) { mutableStateOf(false) }

    content()

    if (showGuide) {
        val activeGuide = if (currentStep == 2 && displayGuide.nextStep != null) {
            displayGuide.nextStep
        } else {
            displayGuide
        }
        val hasNextStep = currentStep == 1 && displayGuide.nextStep != null
        ...
        AlertDialog(
            ...
            text = {
                Column {
                    activeGuide.warning?.let { warning ->
                        Text(
                            text = warning,
                    ...
                    Text(activeGuide.message)
                    activeGuide.acknowledgement?.let { acknowledgement ->
                        Spacer(modifier = Modifier.height(16.dp))
