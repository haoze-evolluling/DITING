...
/**
 * Localization entries for network diagnostics (current network info, DNS query speed test, ping test, DNS resolution lookup, traceroute).
 */
internal fun translateNetworkToolsExact(text: String): String? = when (text) {
    ...
    "通过 Go 隧道解析" -> "Resolve via Go tunnel"
    "域名经运行中的 Go 隧道解析（应用覆写与过滤规则），探测仍走物理网络" ->
        "Domains resolve through the running Go tunnel (rewrites and filter rules apply); probes still traverse the physical network"
    "经运行中的 Go 隧道解析（应用覆写与过滤规则），忽略下方服务器选择" ->
        "Resolves through the running Go tunnel (rewrites and filter rules apply), ignoring the server selection below"
    "VPN 未运行，无法通过 Go 隧道解析" -> "VPN is not running; cannot resolve via the Go tunnel"
    "已通过 Go 隧道解析 · 来源：覆写规则" -> "Resolved via Go tunnel · Source: rewrite rule"
    "已通过 Go 隧道解析 · 来源：上游 DNS" -> "Resolved via Go tunnel · Source: upstream DNS"
    "来源：覆写规则" -> "Source: rewrite rule"
    "来源：上游 DNS" -> "Source: upstream DNS"
    "隧道解析失败" -> "Tunnel resolution failed"
    "Go 隧道未运行" -> "The Go tunnel is not running"
    "解析结果无效" -> "Invalid resolution result"
    "Go 隧道" -> "Go tunnel"
    else -> null
}
...
internal fun translateNetworkToolsPattern(text: String): String? = when {
    ...
    text.startsWith("已通过 Go 隧道解析") -> text
        .replace("已通过 Go 隧道解析 · 来源：", "Resolved via Go tunnel · Source: ")
        .replace("覆写规则", "rewrite rule")
        .replace("上游 DNS", "upstream DNS")
    text.startsWith("来源：") -> text
        .replace("来源：", "Source: ")
        .replace("覆写规则", "rewrite rule")
        .replace("上游 DNS", "upstream DNS")
        .replace("；已被规则拦截", "; blocked by rule")
        .replace("；隧道解析未返回记录", "; tunnel returned no records")
    text.startsWith("已被规则拦截") -> text.replace("已被规则拦截", "Blocked by rule")
    text.startsWith("隧道解析未返回记录") -> text.replace("隧道解析未返回记录", "Tunnel resolution returned no records")
    text.startsWith("域名已被拦截规则拦截") -> text.replace("域名已被拦截规则拦截", "Domain is blocked by a rule")
    text.startsWith("隧道解析未返回地址") -> text.replace("隧道解析未返回地址", "Tunnel resolution returned no address")
    else -> null
}
