...
/**
 * Route tracing tool (network diagnostics).
 *
 * Probes hop by hop with increasing TTL via the system ping / ping6 -t flag,
 * discovering each hop along the path from the "Time to live exceeded"
 * replies returned by intermediate routers, recording their addresses and
 * response latencies — no root access or separate traceroute binary needed.
 * Like DnsLatencyTester, the app's own traffic is excluded from the VPN, so
...
object NetworkTraceRouteTool {
        ...
        return withContext(Dispatchers.IO) {
            try {
                withTimeout((maxHops.coerceIn(1, MAX_HOPS_LIMIT) * (timeoutSeconds + 2) + 15) * 1000L) {
                    runTrace(sanitized, maxHops.coerceIn(1, MAX_HOPS_LIMIT), timeoutSeconds, addressOverride, onHop)
                }
    ...
    private suspend fun runTrace(
        ...
        val address = runCatching { InetAddress.getByName(addressOverride ?: target) }.getOrNull()
    ...
    /** Probes hop by hop; returns null when the destination was reached, otherwise a failure reason (completed hops stay in the list). */
    ...
    // Matches "64 bytes from 1.2.3.4: icmp_seq=1 ttl=57 time=12.3 ms" (address may be IPv6)
    private val REPLY_REGEX = Regex("from\\s+(\\S+):\\s+(?:icmp_seq|seq)=\\d+.*?time=([0-9.]+)\\s*ms")

    // Matches "From 1.2.3.4: icmp_seq=1 Time to live exceeded" (IPv4, colon-delimited)
    // or     "From 2001:db8::1 icmp_seq=1 Time exceeded: Hop limit" (IPv6, no colon)
