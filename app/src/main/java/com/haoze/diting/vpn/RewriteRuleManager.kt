...
class RewriteRuleManager(
    ...
    @Volatile private var subscriptionIpRules: Map<String, List<String>> = emptyMap()
    ...
    suspend fun refreshCache(rebuildSubscriptionIndex: Boolean = false) {
        ...
        val indexedSubscriptionRules = if (subscriptionIndex != null) dao.enabledSubscriptionRules() else emptyList()
        subscriptionCnameRules = buildMap {
            indexedSubscriptionRules
                .filter { it.targetType == RewriteTargetType.CNAME }
                .forEach { put(it.pattern, it.targetValue) }
        }
        subscriptionIpRules = indexedSubscriptionRules
            .filter { it.targetType == RewriteTargetType.IPV4 || it.targetType == RewriteTargetType.IPV6 }
            .groupBy({ it.pattern }, { it.targetValue })
    ...
    /**
     * Exports all enabled IPv4/IPv6 rewrite answers (manual rules are written
     * last so they take precedence over subscriptions) for the DNS-stage
     * rewriter in the Go engine.
     */
    fun ipRewrites(): Map<String, List<String>> = buildMap {
        subscriptionIpRules.forEach { (pattern, ips) -> put(pattern, ips) }
        subscriptionFallbackRules.forEach { (pattern, answers) ->
            val ips = answers.ipTargets()
    ...
    suspend fun clearAll() {
        ...
        subscriptionCnameRules = emptyMap()
        subscriptionIpRules = emptyMap()
...
private fun List<com.haoze.diting.data.dao.EnabledRewriteRule>.toAnswerMap(): Map<String, Set<RewriteAnswer>> =
    groupBy({ it.pattern }, { RewriteAnswer(it.targetType, it.targetValue) }).mapValues { it.value.toSet() }

private fun Set<RewriteAnswer>.ipTargets(): List<String> =
    filter { it.targetType != RewriteTargetType.CNAME }.map { it.targetValue }.distinct()
...
private suspend fun RewriteRuleDao.forEachSubscriptionRulePage(
    consume: (com.haoze.diting.data.dao.EnabledRewriteRule) -> Unit
) = forEachKeysetPage(
