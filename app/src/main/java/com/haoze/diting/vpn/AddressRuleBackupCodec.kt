...
object AddressRuleBackupCodec {
    private const val FORMAT = "diting_address_rule_backup"
    private const val LEGACY_FORMAT = "dnssr_https_rule_backup"
    ...
    fun decode(content: String): AddressRuleBackup {
        ...
        require(format == FORMAT || format == "dnssr_address_rule_backup" || format == LEGACY_FORMAT) { "不是地址规则备份文件" }
        val version = root.optInt("version", -1)
        require(version == VERSION || (format == LEGACY_FORMAT && version in 1..2)) { "不支持的地址规则备份版本" }
        val sourceStr = root.optString("source")
        val source = AddressRuleBackupSource.entries.firstOrNull { it.storageValue == sourceStr }
    ...
    private fun readRules(rules: JSONObject, key: String, allow: Boolean): List<String> = buildList {
        val values = rules.optJSONArray(key) ?: return@buildList
