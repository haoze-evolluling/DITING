...
/**
 * DNS resolution lookup tool (network diagnostics).
 *
 * Sends a standard A/AAAA query to the given DNS server and fully parses the
 * answer records, showing resolved IPs, CNAME/NS and other records, TTLs, and
 * the response status. Does not depend on VpnService; reuses DnsMessageUtils
 * for message construction and PlainDnsTransport for transport (UDP with a
...
object DnsLookupTool {
    ...
    /**
     * Maps a Go-tunnel resolution (TunnelDiagnosticsResolver) into a lookup
     * result so the DNS tool can display tunnel-path answers, including the
     * decision source (rewrite rule / upstream / blocked).
     */
    fun tunnelLookup(
        host: String,
        recordType: RecordType,
        tunnelResult: TunnelDiagnosticsResolver.Result,
        elapsedMs: Long
    ): Result {
        val sourceLabel = tunnelResult.sourceLabel()
        val message = when (tunnelResult.source) {
            TunnelDiagnosticsResolver.Source.BLOCKED ->
                "已被规则拦截${tunnelResult.reason?.let { "：$it" }.orEmpty()}"
            TunnelDiagnosticsResolver.Source.NODATA ->
                "隧道解析未返回记录${tunnelResult.reason?.let { "（$it）" }.orEmpty()}"
            TunnelDiagnosticsResolver.Source.ERROR -> tunnelResult.errorLabel()
            else -> null
        }
        val records = tunnelResult.ips.mapIndexed { index, ip ->
            Record(host, recordType.label, tunnelResult.ttls.getOrNull(index)?.toLong() ?: 0L, ip)
        }
        return Result(
            queryName = host,
            recordTypeLabel = recordType.label,
            server = "Go 隧道",
            records = records,
            resolvedAddresses = tunnelResult.ips,
            elapsedMs = elapsedMs,
            rcodeLabel = null,
            success = tunnelResult.ips.isNotEmpty(),
            message = listOfNotNull(
                sourceLabel.takeIf { it.isNotEmpty() }?.let { "来源：$it" },
                message
...
/**
 * Lightweight DNS answer-record parser covering the Answer / Authority /
 * Additional sections, with name-compression pointer support. Only parses the
 * common record types the debug tool displays; on malformed input it returns
...
private object DnsRecordParser {
    ...
    /** Returns (name, offset just past the name field), or null on malformed input. */
