...
import com.haoze.diting.data.dao.EnabledRewriteRule
...
internal class MappedSubscriptionRewriteIndex private constructor(
    ...
    private val parsedAnswers: Array<RewriteAnswer>
) : AutoCloseable {
    ...
    private fun readAnswers(start: Int, count: Int): Set<RewriteAnswer> = buildSet {
        for (index in start until start + count) {
            ...
            if (targetId in parsedAnswers.indices) {
                add(parsedAnswers[targetId])
            }
        }
    }
    ...
    companion object {
        private const val TAG = "MappedRewriteIndex"
        ...
        suspend fun compileAndLoad(
            target: File,
            forEachRule: suspend ((EnabledRewriteRule) -> Unit) -> Unit
        ): MappedSubscriptionRewriteIndex? {
        ...
        fun load(target: File): MappedSubscriptionRewriteIndex {
            ...
            try {
                ...
                val parsedAnswers = Array(answers) {
                    ...
                    copy.position(cursor + 8)
                    copy.get(typeBytes)
                    copy.get(valBytes)
                    cursor += 8 + typeLength + valueLength
                    RewriteAnswer(typeBytes.toString(Charsets.UTF_8), valBytes.toString(Charsets.UTF_8))
                }

                return MappedSubscriptionRewriteIndex(file, data, nodes, edges, answers, bloomBits, HEADER_SIZE, nodeOffset, edgeOffset, labelOffset, targetIdOffset, answerOffset, parsedAnswers)
        ...
        private suspend fun compile(
            ...
            forEachRule { rule ->
                ...
                while (end > 0) {
                    ...
                    node = node.children.getOrPut(label) { RewriteBuildNode() }
                    end = dot
                }
            ...
            fun collectBloom(node: RewriteBuildNode, reverseLabels: MutableList<String>) {
                if (node.answers.isNotEmpty()) addBloom(reverseLabels.asReversed().joinToString("."))
                node.children.forEach { (label, child) ->
                    reverseLabels += label
                    collectBloom(child, reverseLabels)
                    reverseLabels.removeAt(reverseLabels.lastIndex)
                }
            }
            collectBloom(root, mutableListOf())
