...
// When active, non-DNS packets are silently dropped: only DNS queries are
// intercepted for ad-blocking, and all other traffic uses the normal network.
// Serves as the default when no proxy is configured.
...
// HandlePacket drops non-DNS packets in DNS-only mode. The TUN only receives
// DNS queries (routed via the fake DNS server 10.0.0.1), so this is rarely
// called.
