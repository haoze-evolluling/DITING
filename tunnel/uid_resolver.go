...
// replacement is the framework API
...
// a tun2socks flow ID and calls the registered resolver, hiding the
// gVisor address-type conversion from handler code.
//
// Direction: tun2socks names the fields from the stack's point of view,
// so "local" in TransportEndpointID is the server side and "remote" is
// the app's ephemeral socket; the Android API wants the app's actual
// socket (tun2socks "remote") looked up against the server it was
// connecting to (tun2socks "local").
