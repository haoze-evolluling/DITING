...
//   2. Auto-blacklist → domains that failed the TLS handshake are permanently
//     passed through (cert pinning detected).
//   3. SNI keywords   → domains containing sensitive keywords (bank, pay,
...
// Catches banking, authentication, and payment services dynamically without
// maintaining a huge hardcoded domain list.
...
// minimalPassthroughSuffixes is intentionally empty so that all default and custom whitelist
// domains are unified and controlled dynamically from the Whitelist module.
