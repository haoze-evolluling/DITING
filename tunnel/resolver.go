...
func NewResolver(protectFn func(fd int) bool) *Resolver {
	...
	return &Resolver{
		...
		outbound:        outbound,
		httpClient:      buildDoHClient(outbound),
	}
}
...
func (r *Resolver) setOutbound(outbound flowOutbound) {
	...
	r.outbound = outbound
	r.httpClient = buildDoHClient(outbound)
	r.mu.Unlock()
	if oldClient != nil {
...
func buildDoHClient(outbound flowOutbound) *http.Client {
	transport := &http.Transport{
		DialContext: func(ctx context.Context, network, address string) (net.Conn, error) {
			return outbound.DialTCP(ctx, address)
		},
...
func (r *Resolver) queryPlain(rawQuery []byte, server string) ([]byte, error) {
	...
	if !outbound.SupportsUDP() {
		return queryPlainTCP(ctx, outbound, rawQuery, server)
	}
	conn, err := outbound.DialUDP(ctx, server)
	if err != nil {
...
func queryPlainTCP(ctx context.Context, outbound flowOutbound, rawQuery []byte, server string) ([]byte, error) {
	conn, err := outbound.DialTCP(ctx, server)
	if err != nil {
		return nil, fmt.Errorf("plain TCP dial: %w", err)
	}
	defer conn.Close()
	_ = conn.SetDeadline(time.Now().Add(queryTimeoutPlain))
	length := make([]byte, 2)
...
func (r *Resolver) queryDoT(rawQuery []byte, server string) ([]byte, error) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), connectTimeout)
	defer cancel()
	conn, err := outbound.DialTCP(ctx, net.JoinHostPort(host, port))
	if err != nil {
...
func (r *Resolver) getOrCreateQUICConn(host, port string) (quic.Connection, error) {
	...
	var udpAddr net.Addr = unresolvedUDPAddr(net.JoinHostPort(host, port))
	if _, proxied := outbound.(*proxyFlowOutbound); !proxied {
		resolved, resolveErr := net.ResolveUDPAddr("udp", net.JoinHostPort(host, port))
		if resolveErr != nil {
			udpConn.Close()
